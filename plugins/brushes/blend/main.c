                ...
                if (is_eraser) {
                    ...
                    if (tex_fb && tex_fb->width > 0 && tex_mode > 0 && tex_strength > 0) {
                        ...
                        if (tex_mode == 1) {
                            ...
                            cur_wet = (cur_wet * (lum * tex_strength + 255 * (100 - tex_strength))) / 25500;
                        } else if (tex_mode == 2) {
                            uint32_t cr = col & 0xFF, cg = (col >> 8) & 0xFF, cb = (col >> 16) & 0xFF;
                            col = (col & 0xFF000000) | (((cb * tb) / 255) << 16) | (((cg * tg) / 255) << 8) | ((cr * tr) / 255);
                        }
                    }
...
void on_message(int32_t from_id, int32_t len) {
    ...
    if (type == MSG_BRUSH_STROKE && len >= sizeof(wesenho_stroke_msg_t)) {
        ...
        if (smsg->state == STROKE_START) {
            stamp_blend(fb, tex_fb, smsg->x, smsg->y, smsg->color, smsg->is_eraser);
        } else {
            ...
            for (int i = 0; i <= steps; i++) {
                ...
                stamp_blend(fb, tex_fb, cx, cy, smsg->color, smsg->is_eraser);
            }
        }
    }
}
