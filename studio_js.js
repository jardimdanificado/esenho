        ...
        if (activeTool === 'select') {
          ...
          if (activeResizeHandle && initialBounds && initialObjectState) {
            ...
            if (Array.isArray(initialObjectState)) {
              ...
              const isDiagonal = ['nw', 'ne', 'se', 'sw'].includes(activeResizeHandle);
              if (isDiagonal) {
                const origW = Math.max(1, initialBounds.width || (initialBounds.maxX - initialBounds.minX) || 1);
                const origH = Math.max(1, initialBounds.height || (initialBounds.maxY - initialBounds.minY) || 1);
                const denom = Math.max(1, origW * origW + origH * origH);
                let s = 1;
                if (activeResizeHandle === 'se') {
                  s = 1 + (dX * origW + dY * origH) / denom;
                ...
                const minS = Math.max(2 / origW, 2 / origH);
                s = Math.max(minS, s);
                const newW = origW * s;
                const newH = origH * s;
              ...
              if (obj && initialOrigin) {
                ...
                if (isDiagonal) {
                  ...
                  let s = 1;
                  if (activeResizeHandle === 'se') {
                    s = 1 + (dLocalX * origW + dLocalY * origH) / denom;
                  } else if (activeResizeHandle === 'sw') {
                    s = 1 + (-dLocalX * origW + dLocalY * origH) / denom;
                  } else if (activeResizeHandle === 'ne') {
                    s = 1 + (dLocalX * origW - dLocalY * origH) / denom;
