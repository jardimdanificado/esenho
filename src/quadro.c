...
typedef struct {
    int32_t  id;
    int32_t  layer_idx;
    int32_t  shape_type;        /* 0=path, 1=rect, 2=ellipse, 3=poly */
    uint32_t color;             /* stroke color */
    uint32_t fill_color;        /* fill color (0 = transparent) */
    int32_t  stroke_width;      /* explicit width if > 0 */
    int32_t  eraser;
    int32_t  closed;
...
static void ensure_vstroke_capacity(w_vlayer_internal_t *vl, int min_cap) {
    ...
    for (int i = vl->capacity; i < new_cap; i++) {
        new_strokes[i].id = 0;
        new_strokes[i].shape_type = 0;
        new_strokes[i].fill_color = 0;
        new_strokes[i].stroke_width = 0;
...
static w_vstroke_internal_t *find_vstroke(int32_t layer_idx, int32_t obj_id, int32_t *out_layer_idx, int32_t *out_stroke_idx) {
    if (!vlayers || vlayer_capacity <= 0) return 0;
    int start_l = (layer_idx >= 0) ? layer_idx : 0;
    int end_l = (layer_idx >= 0) ? (layer_idx + 1) : vlayer_capacity;
    if (start_l < 0) start_l = 0;
    if (end_l > vlayer_capacity) end_l = vlayer_capacity;

    for (int l = start_l; l < end_l; l++) {
        w_vlayer_internal_t *vl = &vlayers[l];
        for (int s = 0; s < vl->count; s++) {
            if (vl->strokes[s].id == obj_id) {
                if (out_layer_idx) *out_layer_idx = l;
                if (out_stroke_idx) *out_stroke_idx = s;
                return &vl->strokes[s];
            }
        }
    }
...
static void recalculate_bbox(w_vstroke_internal_t *st) {
    if (!st || st->point_count <= 0) {
        if (st) { st->min_x = 0; st->min_y = 0; st->max_x = 0; st->max_y = 0; }
        return;
    }
    st->min_x = 999999;
    st->min_y = 999999;
    st->max_x = -999999;
    st->max_y = -999999;
    for (int i = 0; i < st->point_count; i++) {
        if (st->points[i].x < st->min_x) st->min_x = st->points[i].x;
        if (st->points[i].y < st->min_y) st->min_y = st->points[i].y;
        if (st->points[i].x > st->max_x) st->max_x = st->points[i].x;
        if (st->points[i].y > st->max_y) st->max_y = st->points[i].y;
    }
}

static inline int pt_in_poly(int x, int y, w_vpoint_t *pts, int count) {
    if (count < 3) return 0;
    int inside = 0;
    for (int i = 0, j = count - 1; i < count; j = i++) {
        if (((pts[i].y > y) != (pts[j].y > y)) &&
            (x < (pts[j].x - pts[i].x) * (y - pts[i].y) / ((pts[j].y - pts[i].y) == 0 ? 1 : (pts[j].y - pts[i].y)) + pts[i].x)) {
            inside = !inside;
        }
    }
    return inside;
...
W_EXPORT int32_t w_vector_stroke_begin(uint32_t color, int32_t eraser) {
    ...
    st->layer_idx = lidx;
    st->shape_type = W_VSHAPE_PATH;
    st->color = color;
    st->fill_color = 0;
    st->stroke_width = brush_config.size;
...
W_EXPORT int32_t w_vector_create_shape(int32_t type, int32_t x, int32_t y, int32_t w, int32_t h, uint32_t stroke_color, uint32_t fill_color) {
    ...
    st->fill_color = fill_color;
    st->stroke_width = brush_config.size > 0 ? brush_config.size : 2;
    st->eraser = 0;
    st->closed = 1;
    st->config = brush_config;
    st->point_count = 0;
    ...
    curr_vstroke = st;
    if (type == W_VSHAPE_RECT) {
        ensure_vpoint_capacity(st, 4);
        w_vector_stroke_add_point(x, y, 1000, 0, 0);
        w_vector_stroke_add_point(x + w, y, 1000, 0, 0);
        w_vector_stroke_add_point(x + w, y + h, 1000, 0, 0);
        w_vector_stroke_add_point(x, y + h, 1000, 0, 0);
    } else if (type == W_VSHAPE_ELLIPSE) {
        int rx = w / 2, ry = h / 2;
        int cx = x + rx, cy = y + ry;
        ensure_vpoint_capacity(st, 16);
        for (int i = 0; i < 16; i++) {
            int deg = (i * 360) / 16;
            int sin_v = 0, cos_v = 0;
            w_sincos_deg(deg, &sin_v, &cos_v);
            int px = cx + (rx * cos_v) / 1024;
            int py = cy + (ry * sin_v) / 1024;
            w_vector_stroke_add_point(px, py, 1000, 0, 0);
        }
    } else {
        ensure_vpoint_capacity(st, 4);
        w_vector_stroke_add_point(x, y, 1000, 0, 0);
...
W_EXPORT int32_t w_vector_hit_test_object(int32_t layer_idx, int32_t x, int32_t y, int32_t tolerance) {
    ...
    w_vlayer_internal_t *vl = &vlayers[lidx];
    if (vl->count <= 0 || !vl->strokes) return 0;

    int tol_sq = tolerance * tolerance;
    for (int s = vl->count - 1; s >= 0; s--) {
        w_vstroke_internal_t *st = &vl->strokes[s];
        if (st->point_count <= 0 || !st->points) continue;

        if (x < st->min_x - tolerance || x > st->max_x + tolerance ||
            y < st->min_y - tolerance || y > st->max_y + tolerance) {
            continue;
        }

        if (st->closed && pt_in_poly(x, y, st->points, st->point_count)) {
            return st->id;
        }

        for (int p = 0; p < st->point_count; p++) {
            if (dist_sq(x, y, st->points[p].x, st->points[p].y) <= tol_sq) {
                return st->id;
            }
        }
    }
...
W_EXPORT int32_t w_vector_hit_test_node(int32_t layer_idx, int32_t obj_id, int32_t x, int32_t y, int32_t radius) {
    init_surface_if_needed();
    w_vstroke_internal_t *st = find_vstroke(layer_idx, obj_id, 0, 0);
    if (!st || st->point_count <= 0 || !st->points) return -1;

    int rad_sq = radius * radius;
    for (int p = 0; p < st->point_count; p++) {
        if (dist_sq(x, y, st->points[p].x, st->points[p].y) <= rad_sq) {
            return p;
        }
    }
    return -1;
...
W_EXPORT int32_t w_vector_set_point(int32_t layer_idx, int32_t obj_id, int32_t pt_idx, int32_t x, int32_t y, int32_t pressure) {
    init_surface_if_needed();
    w_vstroke_internal_t *st = find_vstroke(layer_idx, obj_id, 0, 0);
    if (!st || pt_idx < 0 || pt_idx >= st->point_count) return 0;
    st->points[pt_idx].x = x;
    st->points[pt_idx].y = y;
    if (pressure >= 0) st->points[pt_idx].pressure = pressure;
    recalculate_bbox(st);
    return 1;
}
...
W_EXPORT int32_t w_vector_insert_point(int32_t layer_idx, int32_t obj_id, int32_t pt_idx, int32_t x, int32_t y, int32_t pressure) {
    ...
    w_vstroke_internal_t *st = find_vstroke(layer_idx, obj_id, 0, 0);
    if (!st) return 0;
    if (pt_idx < 0) pt_idx = 0;
    if (pt_idx > st->point_count) pt_idx = st->point_count;

    ensure_vpoint_capacity(st, st->point_count + 1);
    for (int i = st->point_count; i > pt_idx; i--) {
        st->points[i] = st->points[i - 1];
    }
    st->points[pt_idx].x = x;
    st->points[pt_idx].y = y;
    st->points[pt_idx].pressure = (pressure >= 0) ? pressure : 1000;
    st->points[pt_idx].tilt_x = 0;
    st->points[pt_idx].tilt_y = 0;
    st->point_count++;
    recalculate_bbox(st);
    return 1;
}
...
W_EXPORT int32_t w_vector_delete_point(int32_t layer_idx, int32_t obj_id, int32_t pt_idx) {
    ...
    w_vstroke_internal_t *st = find_vstroke(layer_idx, obj_id, 0, 0);
    if (!st || pt_idx < 0 || pt_idx >= st->point_count) return 0;
    for (int i = pt_idx; i < st->point_count - 1; i++) {
        st->points[i] = st->points[i + 1];
    }
    st->point_count--;
    recalculate_bbox(st);
...
W_EXPORT int32_t w_vector_transform_object(int32_t layer_idx, int32_t obj_id, int32_t dx, int32_t dy, int32_t scale_pct, int32_t rot_deg) {
    init_surface_if_needed();
    w_vstroke_internal_t *st = find_vstroke(layer_idx, obj_id, 0, 0);
    if (!st || st->point_count <= 0) return 0;

    int cx = (st->min_x + st->max_x) / 2;
    int cy = (st->min_y + st->max_y) / 2;

    int sin_v = 0, cos_v = 1024;
    if (rot_deg != 0) {
        w_sincos_deg(rot_deg, &sin_v, &cos_v);
    }

    for (int p = 0; p < st->point_count; p++) {
        int px = st->points[p].x - cx;
        int py = st->points[p].y - cy;
        ...
        if (rot_deg != 0) {
            int rx = (px * cos_v - py * sin_v) / 1024;
            int ry = (px * sin_v + py * cos_v) / 1024;
            px = rx;
            py = ry;
        }

        st->points[p].x = px + cx + dx;
        st->points[p].y = py + cy + dy;
    }
    recalculate_bbox(st);
    return 1;
}
...
W_EXPORT int32_t w_vector_delete_object(int32_t layer_idx, int32_t obj_id) {
    init_surface_if_needed();
    int lidx = 0, sidx = 0;
    w_vstroke_internal_t *st = find_vstroke(layer_idx, obj_id, &lidx, &sidx);
    if (!st) return 0;
    w_vlayer_internal_t *vl = &vlayers[lidx];
    for (int i = sidx; i < vl->count - 1; i++) {
        vl->strokes[i] = vl->strokes[i + 1];
    }
    vl->count--;
    return 1;
}
...
W_EXPORT void w_vector_replay_layer(int32_t layer_idx, int32_t scale_pct, int32_t off_x, int32_t off_y) {
    ...
    int lw = 0, lh = 0;
    uint32_t *pix = get_current_draw_target(&lw, &lh);
    ...
    for (int s = 0; s < vl->count; s++) {
        ...
        /* Optional Polygon / Shape Fill */
        if (st->fill_color != 0 && st->closed && count >= 3 && pix && lw > 0 && lh > 0) {
            poly_count = 0;
            for (int p = 0; p < count && p < MAX_POLY; p++) {
                poly_x[poly_count] = (st->points[p].x * scale_pct) / 100 + off_x;
                poly_y[poly_count] = (st->points[p].y * scale_pct) / 100 + off_y;
                poly_count++;
            }
            fill_polygon(pix, lw, lh, st->fill_color, 0);
            poly_count = 0;
        }

        /* Stroke Outline */
        if ((st->color >> 24) > 0 || st->eraser) {
            brush_config = st->config;
            int orig_size = st->stroke_width > 0 ? st->stroke_width : st->config.size;
            ...
            if (count == 1) {
                ...
                if (st->closed && count >= 3) {
                    int px = (st->points[count - 1].x * scale_pct) / 100 + off_x;
                    int py = (st->points[count - 1].y * scale_pct) / 100 + off_y;
                    int cx = (st->points[0].x * scale_pct) / 100 + off_x;
                    int cy = (st->points[0].y * scale_pct) / 100 + off_y;
                    w_brush_stroke_ext(1, cx, cy, px, py, st->color, st->eraser, 1000, 0, 0);
                }

                int last_p = st->closed ? 0 : (count - 1);
                int prev_p = st->closed ? (count - 1) : (count > 1 ? count - 2 : 0);
...
W_EXPORT int32_t w_vector_get_stroke_info(int32_t layer_idx, int32_t stroke_idx, int32_t *out_info) {
    ...
    out_info[16] = st->shape_type;
    out_info[17] = (int32_t)st->fill_color;
    out_info[18] = st->stroke_width;
    out_info[19] = st->min_x;
    out_info[20] = st->min_y;
    out_info[21] = st->max_x;
    out_info[22] = st->max_y;
    return 1;
}
...
/* =========================================================================
 * Complete 2D Animation Engine (Timeline, Tweens, IK, Mesh Warp, Camera)
 * ========================================================================= */
...
typedef struct {
    ...
    int32_t tween_type;
    int32_t x, y;
    int32_t scale_x_pct, scale_y_pct;
    int32_t rot_deg;
    int32_t opacity;
    int32_t z_depth;
    int32_t target_layer_idx;
} w_keyframe_internal_t;

typedef struct {
    int32_t id;
    int32_t track_type;
    int32_t target_layer_idx;
    w_keyframe_internal_t *keyframes;
    int32_t keyframe_count;
    int32_t keyframe_capacity;
} w_anim_track_internal_t;
...
typedef struct {
    ...
    int32_t fps;
    int32_t loop_mode;
    w_anim_track_internal_t *tracks;
    int32_t track_count;
    int32_t track_capacity;
    int32_t onion_skin_enabled;
    int32_t onion_prev_frames;
    int32_t onion_next_frames;
    int32_t onion_tint_alpha;
} w_timeline_internal_t;

static w_timeline_internal_t anim_timeline = {
    .total_frames = 24,
    .current_frame = 0,
    .fps = 24,
    .loop_mode = 1,
    ...
    .track_count = 0,
    .track_capacity = 0,
    .onion_skin_enabled = 0,
    .onion_prev_frames = 1,
    .onion_next_frames = 1,
    .onion_tint_alpha = 90
};

/* --- 2.5D Multiplane Camera State --- */
typedef struct {
    int32_t x, y, z;
    int32_t zoom_pct;
    int32_t rot_deg;
...
static w_camera_internal_t anim_camera = {
    .x = 0, .y = 0, .z = 0,
    .zoom_pct = 100,
    .rot_deg = 0
};
...
W_EXPORT void w_anim_camera_set(int32_t x, int32_t y, int32_t z, int32_t zoom_pct, int32_t rot_deg) {
    ...
    anim_camera.y = y;
    anim_camera.z = z;
    anim_camera.zoom_pct = (zoom_pct > 0) ? zoom_pct : 100;
    anim_camera.rot_deg = rot_deg;
}

W_EXPORT void w_anim_camera_get(int32_t *out_cam) {
    if (!out_cam) return;
    out_cam[0] = anim_camera.x;
    out_cam[1] = anim_camera.y;
    out_cam[2] = anim_camera.z;
    out_cam[3] = anim_camera.zoom_pct;
    out_cam[4] = anim_camera.rot_deg;
}

/* --- Timeline & Track Operations --- */
static void ensure_track_capacity(int min_cap) {
    if (anim_timeline.track_capacity >= min_cap) return;
    int new_cap = anim_timeline.track_capacity ? anim_timeline.track_capacity * 2 : 16;
    while (new_cap < min_cap) new_cap *= 2;
    w_anim_track_internal_t *new_tracks = (w_anim_track_internal_t*)canvas_alloc(new_cap * sizeof(w_anim_track_internal_t));
    for (int i = 0; i < anim_timeline.track_capacity; i++) {
        new_tracks[i] = anim_timeline.tracks[i];
    }
    for (int i = anim_timeline.track_capacity; i < new_cap; i++) {
        new_tracks[i].id = 0;
        new_tracks[i].track_type = 0;
        new_tracks[i].target_layer_idx = 0;
        new_tracks[i].keyframes = 0;
        new_tracks[i].keyframe_count = 0;
        new_tracks[i].keyframe_capacity = 0;
    }
    anim_timeline.tracks = new_tracks;
    anim_timeline.track_capacity = new_cap;
}

static void ensure_keyframe_capacity(w_anim_track_internal_t *tr, int min_cap) {
    if (tr->keyframe_capacity >= min_cap) return;
    int new_cap = tr->keyframe_capacity ? tr->keyframe_capacity * 2 : 8;
    while (new_cap < min_cap) new_cap *= 2;
    w_keyframe_internal_t *new_kfs = (w_keyframe_internal_t*)canvas_alloc(new_cap * sizeof(w_keyframe_internal_t));
    for (int i = 0; i < tr->keyframe_capacity; i++) {
        new_kfs[i] = tr->keyframes[i];
    }
    tr->keyframes = new_kfs;
    tr->keyframe_capacity = new_cap;
}

W_EXPORT void w_anim_init(int32_t total_frames, int32_t fps) {
    init_surface_if_needed();
    anim_timeline.total_frames = (total_frames > 0) ? total_frames : 24;
    anim_timeline.fps = (fps > 0) ? fps : 24;
    anim_timeline.current_frame = 0;
}

W_EXPORT int32_t w_anim_get_total_frames(void) { return anim_timeline.total_frames; }
W_EXPORT void    w_anim_set_total_frames(int32_t total_frames) { anim_timeline.total_frames = total_frames > 0 ? total_frames : 1; }
W_EXPORT int32_t w_anim_get_fps(void) { return anim_timeline.fps; }
W_EXPORT void    w_anim_set_fps(int32_t fps) { anim_timeline.fps = fps > 0 ? fps : 24; }
W_EXPORT int32_t w_anim_get_frame(void) { return anim_timeline.current_frame; }

W_EXPORT int32_t w_anim_track_create(int32_t track_type, int32_t target_layer_idx) {
    init_surface_if_needed();
    ensure_track_capacity(anim_timeline.track_count + 1);
    int idx = anim_timeline.track_count++;
    w_anim_track_internal_t *tr = &anim_timeline.tracks[idx];
    tr->id = idx + 1;
    tr->track_type = track_type;
    tr->target_layer_idx = (target_layer_idx >= 0) ? target_layer_idx : active_layer;
    tr->keyframes = 0;
    tr->keyframe_count = 0;
    tr->keyframe_capacity = 0;
    return idx;
}

W_EXPORT int32_t w_anim_track_get_count(void) {
    return anim_timeline.track_count;
}

W_EXPORT int32_t w_anim_add_keyframe(int32_t track_idx, int32_t frame_idx, int32_t tween_type) {
    init_surface_if_needed();
    if (track_idx < 0 || track_idx >= anim_timeline.track_count) return -1;
    w_anim_track_internal_t *tr = &anim_timeline.tracks[track_idx];
    ensure_keyframe_capacity(tr, tr->keyframe_count + 1);

    int insert_pos = tr->keyframe_count;
    for (int i = 0; i < tr->keyframe_count; i++) {
        if (tr->keyframes[i].frame_index == frame_idx) {
            tr->keyframes[i].tween_type = tween_type;
            return i;
        }
        if (tr->keyframes[i].frame_index > frame_idx) {
            insert_pos = i;
    ...
    for (int i = tr->keyframe_count; i > insert_pos; i--) {
        tr->keyframes[i] = tr->keyframes[i - 1];
    }
    tr->keyframe_count++;

    w_keyframe_internal_t *kf = &tr->keyframes[insert_pos];
    ...
    kf->opacity = 255;
    kf->z_depth = 0;
    kf->target_layer_idx = tr->target_layer_idx;
    return insert_pos;
}
...
W_EXPORT int32_t w_anim_set_keyframe_transform(int32_t track_idx, int32_t kf_idx, int32_t x, int32_t y, int32_t scale_x_pct, int32_t scale_y_pct, int32_t rot_deg, int32_t opacity, int32_t z_depth) {
    init_surface_if_needed();
    if (track_idx < 0 || track_idx >= anim_timeline.track_count) return 0;
    w_anim_track_internal_t *tr = &anim_timeline.tracks[track_idx];
    if (kf_idx < 0 || kf_idx >= tr->keyframe_count) return 0;
    w_keyframe_internal_t *kf = &tr->keyframes[kf_idx];
    kf->x = x;
    kf->y = y;
    kf->scale_x_pct = scale_x_pct;
    kf->scale_y_pct = scale_y_pct;
    kf->rot_deg = rot_deg;
    kf->opacity = (opacity >= 0 && opacity <= 255) ? opacity : 255;
    kf->z_depth = z_depth;
    return 1;
}
...
W_EXPORT int32_t w_anim_get_keyframe_count(int32_t track_idx) {
    if (track_idx < 0 || track_idx >= anim_timeline.track_count) return 0;
    return anim_timeline.tracks[track_idx].keyframe_count;
}

W_EXPORT int32_t w_anim_get_keyframe_info(int32_t track_idx, int32_t kf_idx, int32_t *out_info) {
    if (!out_info || track_idx < 0 || track_idx >= anim_timeline.track_count) return 0;
    w_anim_track_internal_t *tr = &anim_timeline.tracks[track_idx];
    if (kf_idx < 0 || kf_idx >= tr->keyframe_count) return 0;
    w_keyframe_internal_t *kf = &tr->keyframes[kf_idx];
    out_info[0] = kf->frame_index;
    out_info[1] = kf->duration;
    out_info[2] = kf->tween_type;
    out_info[3] = kf->x;
    out_info[4] = kf->y;
    out_info[5] = kf->scale_x_pct;
    out_info[6] = kf->scale_y_pct;
    out_info[7] = kf->rot_deg;
    out_info[8] = kf->opacity;
    out_info[9] = kf->z_depth;
    return 1;
}

W_EXPORT void w_anim_onion_skin(int32_t enabled, int32_t prev_frames, int32_t next_frames, int32_t tint_alpha) {
    anim_timeline.onion_skin_enabled = enabled ? 1 : 0;
    anim_timeline.onion_prev_frames = (prev_frames >= 0) ? prev_frames : 1;
    anim_timeline.onion_next_frames = (next_frames >= 0) ? next_frames : 1;
    anim_timeline.onion_tint_alpha = (tint_alpha >= 0 && tint_alpha <= 255) ? tint_alpha : 90;
    force_composite();
}

/* --- Motion Tween & Multiplane Evaluator --- */
static inline int ease_in_out(int t_1000) {
    /* Cubic hermite S-curve 3t^2 - 2t^3 */
    int t = t_1000;
    if (t <= 0) return 0;
...
W_EXPORT void w_anim_set_frame(int32_t frame_idx) {
    init_surface_if_needed();
    anim_timeline.current_frame = frame_idx;

    for (int t = 0; t < anim_timeline.track_count; t++) {
        w_anim_track_internal_t *tr = &anim_timeline.tracks[t];
        if (tr->keyframe_count <= 0) continue;

        int k_prev = 0, k_next = 0;
        int found = 0;

        for (int k = 0; k < tr->keyframe_count; k++) {
            if (tr->keyframes[k].frame_index <= frame_idx) {
                k_prev = k;
                found = 1;
            }
            if (tr->keyframes[k].frame_index >= frame_idx) {
        ...
        w_keyframe_internal_t *kf0 = &tr->keyframes[k_prev];
        w_keyframe_internal_t *kf1 = &tr->keyframes[k_next];

        int cur_x = kf0->x;
        int cur_y = kf0->y;
        int cur_scale_x = kf0->scale_x_pct;
        int cur_scale_y = kf0->scale_y_pct;
        int cur_rot = kf0->rot_deg;
        int cur_op = kf0->opacity;
        int cur_z = kf0->z_depth;

        if (k_prev != k_next && kf0->tween_type != W_TWEEN_NONE) {
            int span = kf1->frame_index - kf0->frame_index;
            if (span > 0) {
                int progress = ((frame_idx - kf0->frame_index) * 1000) / span;
                if (kf0->tween_type == W_TWEEN_EASE_IN_OUT) {
                    progress = ease_in_out(progress);
                }
                cur_x += ((kf1->x - kf0->x) * progress) / 1000;
                cur_y += ((kf1->y - kf0->y) * progress) / 1000;
                cur_scale_x += ((kf1->scale_x_pct - kf0->scale_x_pct) * progress) / 1000;
                cur_scale_y += ((kf1->scale_y_pct - kf0->scale_y_pct) * progress) / 1000;
                cur_rot += ((kf1->rot_deg - kf0->rot_deg) * progress) / 1000;
                cur_op += ((kf1->opacity - kf0->opacity) * progress) / 1000;
                cur_z += ((kf1->z_depth - kf0->z_depth) * progress) / 1000;
            }
        }

        /* 2.5D Camera Parallax Perspective Factor */
        int depth_factor = 1000;
        int eff_z = cur_z + anim_camera.z;
        if (eff_z > -900) {
            depth_factor = (1000 * 1000) / (1000 + eff_z);
        }
        int cam_dx = (cur_x - anim_camera.x) * depth_factor / 1000;
        int cam_dy = (cur_y - anim_camera.y) * depth_factor / 1000;
        int cam_scale = (cur_scale_x * depth_factor / 1000 * anim_camera.zoom_pct) / 100;
        int cam_rot = cur_rot + anim_camera.rot_deg;

        int target_l = tr->target_layer_idx;
        if (target_l >= 0 && target_l < layer_count && layers[target_l].in_use) {
            layers[target_l].opacity = (uint8_t)cur_op;
            if (tr->track_type == W_TRACK_VECTOR) {
                if (layers[target_l].pixels) {
                    int total = layers[target_l].width * layers[target_l].height;
                    for (int p = 0; p < total; p++) layers[target_l].pixels[p] = 0;
                }
                w_vector_replay_layer(target_l, cam_scale, cam_dx, cam_dy);
            }
        }
    }
    force_composite();
...
/* =========================================================================
 * 2D Skeletal Bones & Cyclic Coordinate Descent IK (CCD-IK)
 * ========================================================================= */
...
typedef struct {
    ...
    int32_t parent_id;
    int32_t length;
    int32_t local_angle;  /* degrees (0..359) */
    int32_t world_x0, world_y0;
    int32_t world_x1, world_y1;
    int32_t world_angle;
} w_bone_internal_t;

typedef struct {
    int32_t id;
    w_bone_internal_t *bones;
    int32_t bone_count;
    int32_t bone_capacity;
} w_armature_internal_t;

static w_armature_internal_t *armatures = 0;
static int32_t armature_count = 0;
static int32_t armature_capacity = 0;

static void ensure_armature_capacity(int min_cap) {
    if (armature_capacity >= min_cap) return;
    int new_cap = armature_capacity ? armature_capacity * 2 : 4;
    while (new_cap < min_cap) new_cap *= 2;
    w_armature_internal_t *new_arms = (w_armature_internal_t*)canvas_alloc(new_cap * sizeof(w_armature_internal_t));
    for (int i = 0; i < armature_capacity; i++) new_arms[i] = armatures[i];
    armatures = new_arms;
    armature_capacity = new_cap;
}

static void ensure_bone_capacity(w_armature_internal_t *arm, int min_cap) {
    if (arm->bone_capacity >= min_cap) return;
    int new_cap = arm->bone_capacity ? arm->bone_capacity * 2 : 8;
    while (new_cap < min_cap) new_cap *= 2;
    w_bone_internal_t *new_bones = (w_bone_internal_t*)canvas_alloc(new_cap * sizeof(w_bone_internal_t));
    for (int i = 0; i < arm->bone_capacity; i++) new_bones[i] = arm->bones[i];
    arm->bones = new_bones;
    arm->bone_capacity = new_cap;
}

static void recalculate_armature_fk(w_armature_internal_t *arm) {
    if (!arm || arm->bone_count <= 0) return;
    for (int i = 0; i < arm->bone_count; i++) {
        w_bone_internal_t *b = &arm->bones[i];
        if (b->parent_id <= 0) {
            b->world_angle = b->local_angle;
            int sin_v = 0, cos_v = 1024;
            w_sincos_deg(b->world_angle, &sin_v, &cos_v);
            b->world_x1 = b->world_x0 + (b->length * cos_v) / 1024;
            b->world_y1 = b->world_y0 + (b->length * sin_v) / 1024;
        } else {
            for (int p = 0; p < arm->bone_count; p++) {
                if (arm->bones[p].id == b->parent_id) {
                    b->world_x0 = arm->bones[p].world_x1;
                    b->world_y0 = arm->bones[p].world_y1;
                    b->world_angle = (arm->bones[p].world_angle + b->local_angle) % 360;
                    int sin_v = 0, cos_v = 1024;
                    w_sincos_deg(b->world_angle, &sin_v, &cos_v);
                    b->world_x1 = b->world_x0 + (b->length * cos_v) / 1024;
                    b->world_y1 = b->world_y0 + (b->length * sin_v) / 1024;
...
W_EXPORT int32_t w_anim_armature_create(void) {
    init_surface_if_needed();
    ensure_armature_capacity(armature_count + 1);
    int idx = armature_count++;
    w_armature_internal_t *arm = &armatures[idx];
    arm->id = idx + 1;
    arm->bones = 0;
    arm->bone_count = 0;
    arm->bone_capacity = 0;
...
W_EXPORT int32_t w_anim_bone_create(int32_t armature_id, int32_t parent_bone_id, int32_t length, int32_t angle_deg) {
    init_surface_if_needed();
    if (armature_id <= 0 || armature_id > armature_count) return -1;
    w_armature_internal_t *arm = &armatures[armature_id - 1];
    ensure_bone_capacity(arm, arm->bone_count + 1);

    int idx = arm->bone_count++;
    w_bone_internal_t *b = &arm->bones[idx];
    b->id = idx + 1;
    b->parent_id = parent_bone_id;
    b->length = length > 0 ? length : 50;
    b->local_angle = angle_deg % 360;
    b->world_x0 = (parent_bone_id <= 0) ? (doc_width / 2) : 0;
    b->world_y0 = (parent_bone_id <= 0) ? (doc_height / 2) : 0;
    recalculate_armature_fk(arm);
    return b->id;
}

W_EXPORT int32_t w_anim_bone_set_angle(int32_t armature_id, int32_t bone_id, int32_t angle_deg) {
    if (armature_id <= 0 || armature_id > armature_count) return 0;
    w_armature_internal_t *arm = &armatures[armature_id - 1];
    for (int i = 0; i < arm->bone_count; i++) {
        if (arm->bones[i].id == bone_id) {
            arm->bones[i].local_angle = angle_deg % 360;
            recalculate_armature_fk(arm);
            return 1;
        }
    }
...
W_EXPORT int32_t w_anim_bone_get_info(int32_t armature_id, int32_t bone_id, int32_t *out_info) {
    if (!out_info || armature_id <= 0 || armature_id > armature_count) return 0;
    w_armature_internal_t *arm = &armatures[armature_id - 1];
    for (int i = 0; i < arm->bone_count; i++) {
        if (arm->bones[i].id == bone_id) {
            w_bone_internal_t *b = &arm->bones[i];
            out_info[0] = b->id;
            out_info[1] = b->parent_id;
            out_info[2] = b->length;
            out_info[3] = b->local_angle;
            out_info[4] = b->world_x0;
            out_info[5] = b->world_y0;
            out_info[6] = b->world_x1;
            out_info[7] = b->world_y1;
            out_info[8] = b->world_angle;
            return 1;
        }
    }
    return 0;
...
W_EXPORT int32_t w_anim_bone_ik_solve(int32_t armature_id, int32_t effector_bone_id, int32_t target_x, int32_t target_y, int32_t iterations) {
    if (armature_id <= 0 || armature_id > armature_count) return 0;
    w_armature_internal_t *arm = &armatures[armature_id - 1];
    if (arm->bone_count <= 0) return 0;

    int eff_idx = -1;
    for (int i = 0; i < arm->bone_count; i++) {
        if (arm->bones[i].id == effector_bone_id) {
            eff_idx = i;
            break;
        }
    }
    if (eff_idx < 0) return 0;

    int iters = (iterations > 0 && iterations <= 50) ? iterations : 10;

    for (int iter = 0; iter < iters; iter++) {
        int cur = eff_idx;
        while (cur >= 0) {
            w_bone_internal_t *b = &arm->bones[cur];
            w_bone_internal_t *eff = &arm->bones[eff_idx];

            int v1_x = eff->world_x1 - b->world_x0;
            int v1_y = eff->world_y1 - b->world_y0;
            int v2_x = target_x - b->world_x0;
            int v2_y = target_y - b->world_y0;

            int a1 = w_atan2_deg(v1_y, v1_x);
            int a2 = w_atan2_deg(v2_y, v2_x);
            int delta = (a2 - a1);
            if (delta > 180) delta -= 360;
            if (delta < -180) delta += 360;

            b->local_angle = (b->local_angle + delta) % 360;
            if (b->local_angle < 0) b->local_angle += 360;
            recalculate_armature_fk(arm);

            if (b->parent_id <= 0) break;
            int next_cur = -1;
            for (int p = 0; p < arm->bone_count; p++) {
                if (arm->bones[p].id == b->parent_id) {
                    next_cur = p;
...
/* =========================================================================
 * 2D Mesh Warp & Free-form Deformation (FFD) Implementation
 * ========================================================================= */

typedef struct {
    int32_t u, v;         /* Original source texture coordinates */
    int32_t x, y;         /* Deformed target coordinates */
    int32_t armature_id;
    int32_t bone_id;
    int32_t weight_pct;
} w_mesh_vertex_t;
...
typedef struct {
    ...
    int32_t cols, rows;
    w_mesh_vertex_t *verts;
    int32_t vert_count;
} w_mesh2d_internal_t;

static w_mesh2d_internal_t *meshes = 0;
static int32_t mesh_count = 0;
static int32_t mesh_capacity = 0;
...
W_EXPORT int32_t w_anim_mesh_create(int32_t width, int32_t height, int32_t cols, int32_t rows) {
    ...
    if (mesh_count >= mesh_capacity) {
        int new_cap = mesh_capacity ? mesh_capacity * 2 : 4;
        w_mesh2d_internal_t *new_m = (w_mesh2d_internal_t*)canvas_alloc(new_cap * sizeof(w_mesh2d_internal_t));
        for (int i = 0; i < mesh_capacity; i++) new_m[i] = meshes[i];
        meshes = new_m;
        mesh_capacity = new_cap;
    }
    int idx = mesh_count++;
    w_mesh2d_internal_t *m = &meshes[idx];
    m->id = idx + 1;
    m->width = width > 0 ? width : doc_width;
    m->height = height > 0 ? height : doc_height;
    m->cols = cols;
    m->rows = rows;
    m->vert_count = (cols + 1) * (rows + 1);
    m->verts = (w_mesh_vertex_t*)canvas_alloc(m->vert_count * sizeof(w_mesh_vertex_t));

    int vi = 0;
    for (int r = 0; r <= rows; r++) {
        for (int c = 0; c <= cols; c++) {
            int px = (c * m->width) / cols;
            int py = (r * m->height) / rows;
            m->verts[vi].u = px;
            m->verts[vi].v = py;
            m->verts[vi].x = px;
            m->verts[vi].y = py;
            m->verts[vi].armature_id = 0;
            m->verts[vi].bone_id = 0;
            m->verts[vi].weight_pct = 0;
            vi++;
        }
    }
    return m->id;
...
W_EXPORT int32_t w_anim_mesh_set_vertex(int32_t mesh_id, int32_t v_idx, int32_t x, int32_t y) {
    if (mesh_id <= 0 || mesh_id > mesh_count) return 0;
    w_mesh2d_internal_t *m = &meshes[mesh_id - 1];
    if (v_idx < 0 || v_idx >= m->vert_count) return 0;
    m->verts[v_idx].x = x;
    m->verts[v_idx].y = y;
    return 1;
}
...
W_EXPORT int32_t w_anim_mesh_bind_bone(int32_t mesh_id, int32_t v_idx, int32_t armature_id, int32_t bone_id, int32_t weight_pct) {
    ...
    w_mesh2d_internal_t *m = &meshes[mesh_id - 1];
    if (v_idx < 0 || v_idx >= m->vert_count) return 0;
    m->verts[v_idx].armature_id = armature_id;
    m->verts[v_idx].bone_id = bone_id;
    m->verts[v_idx].weight_pct = weight_pct;
    return 1;
}
...
W_EXPORT int32_t w_anim_mesh_render(int32_t mesh_id, int32_t src_layer_idx, int32_t dst_layer_idx) {
    ...
    if (mesh_id <= 0 || mesh_id > mesh_count) return 0;
    w_mesh2d_internal_t *m = &meshes[mesh_id - 1];
    int src_l = (src_layer_idx >= 0) ? src_layer_idx : active_layer;
    int dst_l = (dst_layer_idx >= 0) ? dst_layer_idx : active_layer;
    if (src_l < 0 || src_l >= layer_count || !layers[src_l].pixels) return 0;
    if (dst_l < 0 || dst_l >= layer_count || !layers[dst_l].pixels) return 0;

    layer_t *sl = &layers[src_l];
    layer_t *dl = &layers[dst_l];

    /* Forward grid cells as quad-fills */
    int stride = m->cols + 1;
    for (int r = 0; r < m->rows; r++) {
        for (int c = 0; c < m->cols; c++) {
            w_mesh_vertex_t *v00 = &m->verts[r * stride + c];
            w_mesh_vertex_t *v10 = &m->verts[r * stride + c + 1];
            w_mesh_vertex_t *v01 = &m->verts[(r + 1) * stride + c];
            w_mesh_vertex_t *v11 = &m->verts[(r + 1) * stride + c + 1];

            /* Bounding box of quad */
            int min_x = v00->x < v10->x ? v00->x : v10->x;
            if (v01->x < min_x) min_x = v01->x;
            if (v11->x < min_x) min_x = v11->x;
            int max_x = v00->x > v10->x ? v00->x : v10->x;
            if (v01->x > max_x) max_x = v01->x;
            if (v11->x > max_x) max_x = v11->x;

            int min_y = v00->y < v10->y ? v00->y : v10->y;
            if (v01->y < min_y) min_y = v01->y;
            if (v11->y < min_y) min_y = v11->y;
            int max_y = v00->y > v10->y ? v00->y : v10->y;
            if (v01->y > max_y) max_y = v01->y;
            if (v11->y > max_y) max_y = v11->y;

            if (min_x < 0) min_x = 0;
            if (min_y < 0) min_y = 0;
            if (max_x >= dl->width) max_x = dl->width - 1;
            if (max_y >= dl->height) max_y = dl->height - 1;

            int src_mid_x = (v00->u + v11->u) / 2;
            int src_mid_y = (v00->v + v11->v) / 2;
            if (src_mid_x >= 0 && src_mid_x < sl->width && src_mid_y >= 0 && src_mid_y < sl->height) {
                uint32_t color = sl->pixels[src_mid_y * sl->width + src_mid_x];
                if ((color >> 24) > 0) {
                    for (int y = min_y; y <= max_y; y++) {
                        for (int x = min_x; x <= max_x; x++) {
                            dl->pixels[y * dl->width + x] = color;
                        }
                    }
                }
            }
...
/* =========================================================================
 * Symbols / Nested Sub-Timelines Implementation
 * ========================================================================= */
...
static w_symbol_internal_t *symbols = 0;
static int32_t symbol_count = 0;
static int32_t symbol_capacity = 0;

W_EXPORT int32_t w_anim_symbol_create(int32_t total_frames, int32_t loop_mode) {
    init_surface_if_needed();
    if (symbol_count >= symbol_capacity) {
        int new_cap = symbol_capacity ? symbol_capacity * 2 : 4;
        w_symbol_internal_t *new_s = (w_symbol_internal_t*)canvas_alloc(new_cap * sizeof(w_symbol_internal_t));
        for (int i = 0; i < symbol_capacity; i++) new_s[i] = symbols[i];
        symbols = new_s;
        symbol_capacity = new_cap;
    }
    int idx = symbol_count++;
    w_symbol_internal_t *s = &symbols[idx];
    s->id = idx + 1;
    s->total_frames = (total_frames > 0) ? total_frames : 12;
    s->loop_mode = loop_mode;
    s->target_layer_idx = active_layer;
    return s->id;
}

W_EXPORT int32_t w_anim_symbol_instantiate(int32_t symbol_id, int32_t parent_track_idx, int32_t start_frame) {
    init_surface_if_needed();
    if (symbol_id <= 0 || symbol_id > symbol_count) return -1;
